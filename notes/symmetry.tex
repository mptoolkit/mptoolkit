...
We also hope to unify this picture with braiding statistics and particle exchange statistics;
it is not a priori clear that these are the same thing! In particular, I haven't seen yet
any good discussion on the different types of exchange statistics that are possible for the
same particle algebra (eg spinless fermions versus bosons, or even abelian anyons, all
appear to have the same $U(1)$ fusion algebra). For non-abelian anyons, the
$R$ symbols are generally presented as fixed quantitities, but clearly for simple spinless
(eg $U(1)$) particles we have a choice of particle exchange statistics, which appears to be
possible to encode into the $R$ matrix, but this then implies that the $R$ matrix is \emph{not}
fixed by the fusion algebra alone. Is this different in the anyonic case? Eg can we give
non-abelian anyons an additional fermionic phase? I'm thinking that the R matrix for
anyons \emph{is} fixed, up to some braid group that mutually commutes. An example is if we have
anyons with particle conservation,  then we can attach an arbitrary abelian anyonic braid representation
(ie bosonic, fermionic or anyonic phase factor)
to the $U(1)$ part. But typically we \emph{cannot} add such a phase to non-abelian anyons alone,
eg for Fibonacci anyons we have $\tau \times \tau = 1 + \tau$, so no abelian phase factor is possible.
On the other hand, for Ising anyons, it appears at first glance we could attach a phase $-1$
to the $\sigma$ anyon.
