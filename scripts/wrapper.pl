...
# load the arguments
(my $ScriptName, my $ExecName, my $BasePath, my $FileName) = @ARGV;

defined($FileName) || die("Not enough arguments supplied: expected ScriptName, ExecutableName, BasePath, FileName");

# get the preferred queue to use for the restart
#my $PBSQueue = defined($ENV{'RESUB_QUEUE'}) ? $ENV{'RESUB_QUEUE'} : $ENV{'PBS_QUEUE'};
#defined($PBSQueue) || Panic "PBS_QUEUE is undefined.";
...
my $CheckpointExtra = 18000;       # 5 hours
...
my $VMem = ConvertToKB($List{'Resource_List.vmem'});

# get the walltime allocation
my $Walltime = TimeToSecs($List{'Resource_List.walltime'});

# set the JOBFS_CHECKPOINT variable to our allocated jobfs size minus the vmem size (to allow for the metadata)
my $jc = $ReqJobFS - $VMem;
...
# copy the persistent data to jobfs
(-e "$BasePath/$FileName.meta.$Suffix") ||
      Panic "Metadata file $BasePath/$FileName.meta.$Suffix does not exist!";

BulkCopy("/fast/x18/running/$FileName.bin.*.$Suffix", $BinPath, ".$Suffix");

# copy the metadata file
`$CopyCommand $BasePath/$FileName.meta.$Suffix $BinPath/$FileName.meta`;
($? == 0) || Panic("Copy of metadata file returned non-zero exit code of $?");
...
while (1)
{
   ...
   (my $hoursUsed, my $minsUsed, my $secsUsed) = split(':', $List{'resources_used.walltime'});
   ...
   if ($WalltimeRemain < 2 * $CheckpointExtra || $ForceRequeue)
   {
      ...
      my $ExecString = "qsub -q $PBSQueue -l jobfs=$ReqJobFS"."kb,vmem=$VMem"."kb,walltime="
        .SecsToTime($Walltime).",other=not_bigsmp -o $ShortScriptName.o$Suffix -e $ShortScriptName.e$Suffix "
	  ."-N $ShortScriptName.$Suffix -r y $ScriptName";
   ...
   # run the program
   print "executing: $ExecName $BasePath $FileName\n";
   my $exit_value = system($ExecName, $BasePath."/", $FileName);
   ...
   if ($exit_value == 0)
   {
      ...
      `rm -f /fast/x18/running/$FileName.bin.*.$Suffix`;
      `rm -f $BasePath/$FileName.meta.$Suffix`;

      # copy .psi files to $BasePath
      BulkCopy("$BinPath/$FileName.psi.*", "/fast/x18/psi");
   ...
   BulkCopy("$BinPath/$FileName.bin.*", "/fast/x18/running", "", ".$Suffix");

   # copy the metadata file
   if (-e "$BasePath/$FileName.meta.$Suffix") {`rm -f $BasePath/$FileName.meta.$Suffix`};
   (system("$CopyCommand", "$BinPath/$FileName.meta", "$BasePath/$FileName.meta.$Suffix") == 0)
     || Panic("Copy of metadata file returned non-zero exit code of $?");
   ...
   `rm -f /fast/x18/running/$FileName.bin.*.$OldSuffix`;
   `rm -f $BasePath/$FileName.meta.$OldSuffix`;
   ...
   if ($exit_value == 5)     # ran out of jobfs?
   {
      # Increase the jobfs request for next time
      if ($ReqJobFS >= $MaxJobFS) { Panic "Cannot allocate more JOBFS - already at maximum.  Not auto-requeueing." };
      $ReqJobFS = int($ReqJobFS * 1.2);
      if ($ReqJobFS > $MaxJobFS) { $ReqJobFS = $MaxJobFS };
      $ForceRequeue = 1;
   }
   ...
   if ($exit_value == 8)
   {
     ...
     print "Caught SIGUSR2, Increasing vmem from " . $VMem . " to " . $NewVMem . " and requeueing.\n";
     $VMem = $NewVMem;
     $ForceRequeue = 1;
   }
