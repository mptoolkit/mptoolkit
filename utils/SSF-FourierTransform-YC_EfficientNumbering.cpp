...
int main(int argc, char** argv)
{
   ...
   if (argc != 4)
     {
         cerr << "usage: " << basename(argv[0]) << " <width> <grid size> <real-space correlation file> [all required]\n\n"
              ...
              << "           \"YC_EfficientNumbering\" structure into a 2D momentum space (inverse lattice space).\n\n"
   ...
   for (int i_kx = 0; i_kx < GridSize; ++i_kx)
   {
	 ...
	 {
	    // get real-space coordinates for the points n1[i] and n2[i] in a extended 2D flat plane:
	    ...
	    //sum += 2 * cos( kx*(x1-x2) + ky*(y1-y2) ) * (-sqrt(3.0)) * corr[i];
