...
//
// attic/matrixproduct-obsolete/infinitewavefunction.cpp
//
...
LinearWavefunction get_right_orthogonal_wavefunction(InfiniteWavefunction const& Psi)
{
   ...
   LinearWavefunction PsiR = inject_right_old_interface(Psi.Psi, r);
...
void orthogonalize(InfiniteWavefunction& x)
{
   ...
   MPStateComponent Last = PsiL.get_back();  // right-most A-matrix
   ...
   while (Iterations == 20)
   {
      ...
      EtaL = LinearSolvers::Arnoldi(LeftEigen, LeftMultiply(PsiL, x.QShift), Iterations, Tol, false);
   }
   ...
   LinearWavefunction PsiR = inject_right_old_interface(x.Psi, C_left);
   ...
   {
      Iterations = 20; Tol = 1E-12;
      EtaR = LinearSolvers::Arnoldi(RightEigen, RightMultiply(PsiR, x.QShift), Iterations, Tol, false);
      //      DEBUG_TRACE(norm_frob(RightEigen - adjoint(RightEigen)));
...
void orthogonalize(InfiniteWavefunction& x)
{
   ...
   while (Iterations == 20)
   {
      Iterations = 20; Tol = 1E-14;
      RightEigen = 0.5 * (RightEigen + adjoint(RightEigen));
      EtaR = LinearSolvers::Arnoldi(RightEigen, RightMultiply(PsiL, x.QShift), Iterations, Tol, false);
   }
   TRACE(EtaR);
   ...
   PsiL = inject_right_old_interface(PsiL, AInv);
   ...
   PsiL = inject_left_old_interface(I, PsiL);
...
void orthogonalize(InfiniteWavefunction& x)
{
   ...
   x.Psi = inject_left_old_interface(I, x.Psi);
