...
int main(int argc, char** argv)
{
   try
   {
	 ...
	 if ( i%3 != 1 )
         ...
         if ( i%3 == 0 )
           {
             H2 += Sz(0)[i]*Sz(0)[(i-2+u)%u] + 0.5 * (Sp(0)[i]*Sm(0)[(i-2+u)%u] + Sm(0)[i]*Sp(0)[(i-2+u)%u]);
             ++num_bonds_j2;
           }
         else if ( i%3 == 1 )
           {
             H2 += Sz(0)[i]*Sz(1)[i-1] + 0.5 * (Sp(0)[i]*Sm(1)[i-1] + Sm(0)[i]*Sp(1)[i-1]);
             H2 += Sz(0)[i]*Sz(1)[(i+4)%u] + 0.5 * (Sp(0)[i]*Sm(1)[(i+4)%u] + Sm(0)[i]*Sp(1)[(i+4)%u]);
             num_bonds_j2 += 2;
           }
         else if ( i%3 == 2 )
           {
             H2 += Sz(0)[i]*Sz(0)[(i+2)%u] + 0.5 * (Sp(0)[i]*Sm(0)[(i+2)%u] + Sm(0)[i]*Sp(0)[(i+2)%u]);
             ++num_bonds_j2;
           }
      }
