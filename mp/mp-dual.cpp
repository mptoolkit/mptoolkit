...
struct ApplyDual : public boost::static_visitor<void>
{
   ApplyDual(bool SwapLocal_) : SwapLocal(SwapLocal_) {}

   void operator()(InfiniteWavefunctionLeft& Psi) const
   {
      inplace_dual(Psi, SwapLocal);
   }
...
int main(int argc, char** argv)
{
   try
   {
      ...
      bool SwapLocalBasis = false;

      prog_opt::options_description desc("Allowed options", terminal::columns());
      desc.add_options()
         ("help", "show this help message")
         ("swap", prog_opt::bool_switch(&SwapLocalBasis), "Swap the order of the local basis states")
      ...
      if (vm.count("help") > 0 || vm.count("inpsi") < 1)
      {
         ...
         std::cerr << desc << '\n';
         std::cerr << "Dual transformation on the quantum numbers of a wavefunction.\n";
         return 1;
      }
      ...
      boost::apply_visitor(ApplyDual(SwapLocalBasis), Psi.mutate()->Wavefunction());
