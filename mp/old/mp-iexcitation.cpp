...
int main(int argc, char** argv)
{
   ...
   std::vector<double> Norm; // norm of f_k
   std::vector<double> Delta; // the components of the triangular form

   // The initial point in the Krylov sequence
   K.push_back(Op);
   KHerm.push_back(OpHerm);
   Norm.push_back(get_expectation(Phi, Psi.QShift, KHerm[0]*K[0], Rho, Identity, Verbose)[1].real());
   Delta.push_back(0.0);

   // The k=1 component of the Krylov sequence
   // f_1 = iLf_0

   K.push_back(complex(0.0, 1.0) * commutator(H, K[0]));
   //   KHerm.push_back(complex(0.0, -1.0) * commutator(KHerm[0], H));
   KHerm.push_back(complex(0.0, 1.0) * commutator(H, KHerm[0]));
   Norm.push_back(get_expectation(Phi, Psi.QShift, KHerm[1]*K[1], Rho, Identity, Verbose)[1].real());
   Delta.push_back(Norm[1] / Norm[0]);

   std::cout << "Delta[1] = " << Delta[1] << '\n';

   for (int k = 1; k < MaxK; ++k)
   {
      K.push_back(complex(0.0, 1.0) * commutator(H, K[k]) + Delta.back() * K[k-1]);
      KHerm.push_back(complex(0.0, -1.0) * commutator(KHerm[k], H) + Delta.back() * KHerm[k-1]);

#if 0
      TRACE(get_expectation(Phi, Psi.QShift, KHerm[k+1]*K[k+1], Rho, Identity, Verbose));
      TRACE(get_expectation(Phi, Psi.QShift, KHerm[k-1]*K[k-1], Rho, Identity, Verbose));
      TRACE(get_expectation(Phi, Psi.QShift, KHerm[k]*K[k], Rho, Identity, Verbose));
      TRACE(get_expectation(Phi, Psi.QShift, KHerm[k+1]*K[k-1], Rho, Identity, Verbose));
      TRACE(get_expectation(Phi, Psi.QShift, K[k-1]*KHerm[k+1], Rho, Identity, Verbose));
#endif

      Norm.push_back(get_expectation(Phi, Psi.QShift, KHerm[k+1]*K[k+1], Rho, Identity, Verbose)[1].real());
      Delta.push_back(Norm[k+1] / Norm[k]);

      std::cout << "Delta[" << (k+1) << "] = " << Delta[k+1] << '\n';
   }
   ...
   for (unsigned i = 0; i < K.size(); ++i)
   {
      ...
      {
         std::cout << i << ' ' << j << ' ' << get_expectation(Phi, Psi.QShift, KHerm[i]*K[j],
                                                              Rho, Identity, Verbose) << '\n';
         std::cout << i << ' ' << j << ' ' << get_expectation(Phi, Psi.QShift, K[j]*KHerm[i],
                                                              Rho, Identity, Verbose) << '\n';
      }
   }
