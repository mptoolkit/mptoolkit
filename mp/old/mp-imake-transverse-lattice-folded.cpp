...
//
// mp/old/mp-imake-transverse-lattice-folded.cpp
//
...
//      TRACE(MATransverse);
//      TRACE(MATransverseFolded);

      TransOp1.push_front(RightFolded);
      TransOp2.push_front(RightFolded);
      for (int i = 0; i < NumTimesteps; ++i)
      {
         TRACE(i);
         TransOp1.push_front(MATransverseFolded);
         TransOp1.push_front(MBTransverseFolded);

         TransOp2.push_front(MBTransverseFolded);
         TransOp2.push_front(MATransverseFolded);
      }

      // the observable and boundary
      std::list<OperatorComponent> TransOp1Obs = TransOp1;

      SimpleOperator Collapser(MATransverseFolded.Basis1(), make_vacuum_basis(B1.GetSymmetryList()));
      SimpleOperator ObsCollapser(MATransverseFolded.Basis1(), make_vacuum_basis(B1.GetSymmetryList()));
      for (unsigned i = 0; i < B1.size(); ++i)
      {
         SimpleOperator Proj(B1, make_vacuum_basis(B1.GetSymmetryList()), B1[i]);
         Proj(i,0) = 1.0;

         Collapser += tensor_prod(Proj, flip_conj(Proj));
         ObsCollapser += tensor_prod(Obs*Proj, flip_conj(Proj));
      }

      TransOp1.front() = prod(herm(Collapser), TransOp1.front());
      TransOp2.front() = prod(herm(Collapser), TransOp2.front());

      TransOp1Obs.front() = prod(herm(ObsCollapser), TransOp1Obs.front());
      ...
      for (unsigned i = 0; i < TransOperator1.size(); ++i)
      {
         TRACE(i);
