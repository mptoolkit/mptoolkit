...
class Aggregator
{
   ...
   private:
      void RotateRight();
      void ConstructLeft(StatesInfo const& SInfo);

      void RotateLeft();
      void ConstructRight(StatesInfo const& SInfo);
...
void Aggregator::ConstructLeft(StatesInfo const& SInfo)
{
...
void Aggregator::ConstructRight(StatesInfo const& SInfo)
{
   // Construct the mapping from the vectors to the result space
   for (unsigned i = 0; i < Psi.size(); ++i)
   {
...
int main(int argc, char** argv)
{
   try
   {
      ...
      desc.add_options()
         ...
         ("no-h2", prog_opt::bool_switch(&NoH2),
          "don't calculate the exact matrix elements of H^2, use the square of the projected H instead"
      ...
      // load the Lanczos vector
      {
         if (Verbosity >= 2)
            std::cerr << "Loading wavefunction: " << LanczosFile << '\n';
         pvalue_ptr<MPWavefunction> P = pheap::ImportHeap(LanczosFile);
         Psi.push_back(*P);
      }

      // Set up the Hamiltonian
      LoadAttribute(vm, Psi[0], "GroundstateEnergy", GroundstateEnergy, Verbose >= 1);
      LoadAttribute(vm, Psi[0], "Broadening", Broadening, Verbose >= 1);
      ...
      Aggregator Ag(Psi, NumCV, Hamiltonian, Hamiltonian2, Location, SInfo);
