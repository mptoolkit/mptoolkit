...
int main(int argc, char** argv)
{
   try
   {
      ...
      // initialize the 'old' center matrix
      dmrg.ShiftLeftAndExpand();
      MatrixOperator LambdaL = dmrg.Wavefunction().Center();
      MPStateComponent HamL = dmrg.HamMatrices.Right();
      dmrg.ShiftRightAndExpand();

      // initial LambdaR
      MatrixOperator LambdaR = MatchBasis(dmrg.Wavefunction().Basis1(), dmrg.Wavefunction().Basis2());

      SplitOperator Ham2 = prod(dmrg.Ham, dmrg.Ham, dmrg.Ident);

      double InvertPrec = 1e-7;

      for (int Sweeps = 0; Sweeps < NumSweeps; ++Sweeps)
      {
         SweepRight(dmrg, TwoSite, NumIter, SInfo, MixFactor);
         dmrg.ShiftRightAndExpand();
         dmrg.HamMatrices.PushRight(HamL);
         dmrg.Wavefunction().Center() = dmrg.Wavefunction().Center()
            * InvertDiagonal(LambdaR, InvertPrec) * LambdaL;
         double E = dmrg.Solve(NumIter);
         // correct for the energy per unit cell
         // (not yet)
         TruncationInfo States = dmrg.TruncateRight(SInfo, MixFactor);
         LambdaR = dmrg.Wavefunction().Center();
         MPStateComponent HamR = dmrg.HamMatrices.Left();

         SweepLeft(dmrg, TwoSite, NumIter, SInfo, MixFactor);
         dmrg.ShiftLeftAndExpand();
         dmrg.HamMatrices.PushLeft(HamR);
         dmrg.Wavefunction().Center() = LambdaR
            * InvertDiagonal(LambdaL, InvertPrec) * dmrg.Wavefunction().Center();
         E = dmrg.Solve(NumIter);
         States = dmrg.TruncateLeft(SInfo, MixFactor);
         LambdaL = dmrg.Wavefunction().Center();
         HamL = dmrg.HamMatrices.Right();
