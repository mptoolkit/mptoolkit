   ...
   try
   {
      ...
      desc.add_options()
         ...
         ("Frequency,F", prog_opt::value(&FrequenciesAsString),
          "list of frequencies to evaluate the correction vectors")
         ...
         ("krylov-size,k", prog_opt::value<int>(&KrylovSize),
         ...
         ("max-iter,i", prog_opt::value<int>(&MaxIter),
          "maximum number of iterations of the functional minimization")
         ("stop-delta,s", prog_opt::value(&StopDelta),
          "Stopping criteria for the iterations: when the change in "
          "residual in one iteration is smaller than this [default 0]")
         ("no-precondition,p", prog_opt::bool_switch(&NoPrecondition),
          "don't use preconditioning by (w+E-H+i\\eta), instead use (w+E-H)^2")
