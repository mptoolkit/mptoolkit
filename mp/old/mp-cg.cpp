...
struct Precondition
{
   ...
   Precondition(MatrixOperator const& Left_, MatrixOperator const& Right_)
...
double SolverHermitian::Solve(int MaxIterations)
{
   ...
   ConjugateGradient(x.Center(),
                     ...
                     yprime.Center(),
                     Iter, Tol,
                     Precondition(yprime_x.Left(), yprime_x.Right()),
...
int main(int argc, char** argv)
{
   if (argc < 5 || argc > 8)
   {
      ...
      std::cerr << "usage: mp-cg <lattice> <operator> <psi> <rhs> [num-iter] [<maxstates>] [corr]\n";
      return 1;
   }
