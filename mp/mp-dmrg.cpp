...
void SweepLeft(FiniteDMRG& dmrg, StatesInfo const& States, ExpansionInfo const& PreExpand, ExpansionInfo const& PostExpand, int NumStatesKeepNext)
{
   dmrg.StartSweep();
   while (dmrg.Site() > 0)
   {
...
int main(int argc, char** argv)
{
   try
   {
      ...
      desc.add_options()
         ...
         ("pre-sector", prog_opt::value(&PreExpand.ExpandPerSector), "Pre-expansion number of additional states in each quantum number sector [default 1 for fullsvd, rsvd; default 0 for range, random]")
         ...
         ("post-sector", prog_opt::value(&PostExpand.ExpandPerSector), "Post-expansion number of additional states in each quantum number sector [default 1 for fullsvd, rsvd; default 0 for range, random]")
	 ...
	 HamMPO = repeat(HamMPO, Psi.size() / HamMPO.size());
      ...
      for (int Sweeps = 0; Sweeps < MyStates.size(); ++Sweeps)
      {
         ...
         if (Sweeps % 2 == 0)
            SweepLeft(dmrg, States, PreExpand*ModFactor, PostExpand*ModFactor, NumStatesKeepNext);
         else
            SweepRight(dmrg, States, PreExpand*ModFactor, PostExpand*ModFactor, NumStatesKeepNext);
      }
