...
DoDMRGSweepLeft(LinearWavefunction& Psi,
                ...
                double EvolveDelta, double& LastEnergy)
{
...

         {
            ...
            MatrixOperator TruncL = DML.ConstructTruncator(DML.begin(),
                                                           TruncateFixTruncationErrorAbsolute(DML.begin(),
                                                                                              ...
                                                                                              SInfo,
                                                                                              Info));
            std::cout << "A Energy=" <<  (ShowCumulativeEnergy ? (Energy+CumulativeEnergy) : Energy)
                      << " States=" << Info.KeptStates()
                      << " TruncError=" << Info.TruncationError()
                      << " Entropy=" << Info.KeptEntropy()
                      << " Fidelity=" << Fidelity
...
int main(int argc, char** argv)
{
   ...
   try
   {
      ...
      int UnitCellSize;
      bool ShowCumulativeEnergy = false;
      ...
      desc.add_options()
         ...
         ("cumulative", prog_opt::bool_switch(&ShowCumulativeEnergy),
          "Show the cumulative (total) energy, instead of the energy per unit cell")
...
#if 0
      // TODO: this should work OK, but with the new scheme with -b -e it dosn't yet
      ...
      LinearWavefunction LeftWavefunc, RightWavefunc;
      ...
      try
      {
         ...
         for (int i = 0; i < NumSteps; i += 2)
         {
            C = DoDMRGSweepLeft(MyPsi, C, HamMPO, LeftBlock, RightBlock, SInfo, MinIter, NumIter,
                                FidelityAv, TwoSite, MixFactor, EvolveDelta, LastEnergy);

            RightWavefunc.push_front(MyPsi);
            TRACE(RightWavefunc.size() + LeftWavefunc.size());
...

            {
               ...
               MatrixOperator TruncL = DML.ConstructTruncator(DML.begin(),
                                                              TruncateFixTruncationErrorAbsolute(DML.begin(),
                                                                                                 ...
                                                                                                 SInfo,
                                                                                                 Info));
               std::cout << "A Energy=" << (ShowCumulativeEnergy ? (CumulativeEnergy) : Energy)
                         << " States=" << Info.KeptStates()
                         << " TruncError=" << Info.TruncationError()
                         << " Entropy=" << Info.KeptEntropy()
                         << " Fidelity=" << Fidelity
...

            {
               ...
               LeftWavefunc.set_back(prod(LeftWavefunc.get_back(), U));
            }
            ...
            C = DoDMRGSweepRight(C, MyPsi, HamMPO, LeftBlock, RightBlock, SInfo, MinIter, NumIter,
                                 FidelityAv, TwoSite, MixFactor, EvolveDelta, LastEnergy);          // turn around at the right-hand side

            LeftWavefunc.push_back(MyPsi);
            TRACE(RightWavefunc.size() + LeftWavefunc.size());
            ...
            // If we have an odd number of steps, avoid adding a unit cell on the final step.
            if (i == NumSteps-1)
            {
               //               PsiR = C;
               TRACE(ExpanderL.Basis1())(ExpanderL.Basis2())(C.Basis2())(MyPsi.Basis1());
               TRACE(ExpanderR.Basis1())(ExpanderR.Basis2())(MyPsi.Basis2())(C.Basis1());
               //               TRACE(PsiL.Basis1())(PsiL.Basis2());
               //               ExpanderL = ExpanderL * delta_shift(ExpanderR, adjoint(QShift));
               //               TRACE(ExpanderL);
               ...
               //ExpanderL = delta_shift(MatrixOperator::make_identity(PsiR.Basis2()), QShift);
               continue;
            }
            ...
            // truncate the right block
            {
               ...
               MatrixOperator TruncR = DMR.ConstructTruncator(DMR.begin(),
                                                              TruncateFixTruncationErrorAbsolute(DMR.begin(),
                                                                                                 ...
                                                                                                 SInfo,
                                                                                                 Info));
               std::cout << "B Energy=" << (ShowCumulativeEnergy ? (CumulativeEnergy) : Energy)
                         << " States=" << Info.KeptStates()
                         << " TruncError=" << Info.TruncationError()
                         << " Entropy=" << Info.KeptEntropy()
                         << " Fidelity=" << Fidelity
