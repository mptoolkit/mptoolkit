...
int const NumPrimes = 6542;
extern unsigned int Primes[NumPrimes+1];

} // namespace

unsigned int NextPrime(unsigned int p)
{
   unsigned int* Val = std::upper_bound(Primes, Primes+NumPrimes, p);
   if (*Val == p) ++Val;
   if (*Val == 0) --Val;
   return *Val;
}
...
/* list of primes (taken from Michael Fromberger's MPI library) */
unsigned int Primes[NumPrimes+1] = {
    ...
    0xFFA7, 0xFFA9, 0xFFC7, 0xFFD9, 0xFFEF, 0xFFF1, 0x0000
};
